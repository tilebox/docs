...
Use this method for large homogeneous fan-out. The subtasks with the same dependency shape are submitted as a compact group, so a batch can contain many more than 64 tasks. See [dependency groups and fan-out limits](/workflows/concepts/tasks#dependency-groups-and-fan-out-limits) for details.
