...
For larger fan-out, prefer [`submit_subtasks`](/api-reference/python/tilebox.workflows/ExecutionContext.submit_subtasks) when many subtasks share the same dependency shape. See [dependency groups and fan-out limits](/workflows/concepts/tasks#dependency-groups-and-fan-out-limits) for how Tilebox groups subtasks.
...
<ParamField path="depends_on" type="FutureTask | list[FutureTask] | None">
  An optional task or list of tasks already submitted within the same context that this subtask depends on.
</ParamField>
