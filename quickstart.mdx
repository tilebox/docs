...
If you prefer to work locally, follow these steps to get started.
  ...
  <Steps>
    ...
    <Step title="Query Data">
      ...
      ```bash Shell
      tilebox-generate --dataset open_data.copernicus.sentinel2_msi --tilebox-api-key YOUR_TILEBOX_API_KEY
      ```
    </Step>
    <Step title="Query Data">
      Use the datasets client to query data from a dataset.
	...
	func main() {
		...
		job, err := client.Jobs.Submit(ctx, "hello-world", clusterSlug, 0,
			&HelloTask{
				Greeting: "Greetings",
				Name: "Tilebox",
			},
