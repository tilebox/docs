...
<Update label="August 17, 2026">
    ## Concurrent workflow runners in the CLI

    Run `tilebox runner start -n 3` (or `tilebox runner start --runners 3`) to start three independent workflow runners in one CLI process. The process can then handle up to three tasks concurrently. The existing `tilebox parallel` command remains available.
</Update>
