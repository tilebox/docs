...
description: Access and filter data stored in your datasets by time, location, and custom fields, with built-in pagination and progress tracking.
...
[filter by time](/datasets/query/filter-by-time), [filter by custom fields](/datasets/query/filter-by-fields), and for
[Spatio-temporal datasets](/datasets/types/spatiotemporal), optionally [filter by a location in the form of a geometry](/datasets/query/filter-by-location).
...
<Columns cols={2}>
  ...
  <Card
    ...
    icon="filter-list"
    href="/datasets/query/filter-by-fields"
    horizontal
  >
    Filter datapoints by queryable fields in the dataset schema.
  </Card>
  <Card
