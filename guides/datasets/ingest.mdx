...
required packages using your preferred package manager. For new projects, we recommend using [uv](https://docs.astral.sh/uv/).
...
Now we'll create a [Timeseries](/datasets/types/timeseries) dataset with the same schema as the given MODIS dataset.
To do so, we'll use the [Tilebox Console](/console), navigate to `My Datasets` and click `Create Dataset`. We then select
...
<Frame>
  <img src="/assets/guides/ingest/dataset-slug-light.png" alt="Explore the MODIS dataset" className="dark:hidden" />
  <img src="/assets/guides/ingest/dataset-slug-dark.png" alt="Explore the MODIS dataset" className="hidden dark:block" />
</Frame>
