...
```text Prompt
Inspect `tilebox agent-context workflow init --output-schema`. Then initialize a new Tilebox workflow project with `tilebox workflow init --name "<workflow-name>" --json`. After initialization, update the generated runner.py with the workflow tasks.
```
