---
title: Installation
description: Install the Tilebox Go library
icon: download
---

## Package Overview

Tilebox offers a Go SDK for accessing Tilebox services. It additionally includes a CLI tool (tilebox-generate) that can be installed separately.

<CardGroup cols={2}>
  <Card title="tilebox-go" icon="golang" href="https://github.com/tilebox/tilebox-go" horizontal>
    Datasets and workflows client for Tilebox
  </Card>
  <Card title="tilebox-generate" icon="terminal" href="https://github.com/tilebox/tilebox-generate" horizontal>
    CLI tool to generate Tilebox datasets types for Go
  </Card>
</CardGroup>

## Installation

Add `tilebox-go` to your project.

```bash Shell
go get github.com/tilebox/tilebox-go
```

Install `tilebox-generate` CLI on your machine.

```bash Shell
go install github.com/tilebox/tilebox-generate@latest
```
