...
Clusters are a way of grouping [task runners](/workflows/task-runners) together.
This is useful when you have a large number of task runners and you want to manage them as a group.
For example, you might have a cluster defined for task runners that are running on a particular cloud provider, or a cluster
for task runners that share similar hardware.
</Accordion>
...
- Set up different clusters for different environments (e.g. development, production)

Even Task runners belonging to the same cluster may have different capabilities, i.e. different tasks registered that they can run.
Distributing tasks to different runners can also be achieved using only a single cluster, see the
[Distributed Execution](/workflows/task-runners#distributed-execution) example for more details.
...
Task runners can be added to a cluster by specifying the [clusters' slug](#cluster-slug) when [registering a task runner](/workflows/task-runners).
