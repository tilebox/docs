...
func (t *Distributed) Execute(ctx context.Context) error {
...
high_network_speed_runner = client.runner(
    "dev-cluster",
    tasks=[DownloadData, Distributed]
)
...
Now, both `download_task_runner.py` and `gpu_task_runner.py` are started, in parallel, on different machines with the required hardware for each. When `Distributed` is submitted, it executes on one of the two runners, and it's submitted sub-tasks are handled by the appropriate runner.
