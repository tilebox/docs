...
For example, avoid passing a list of 1,000 file paths as a task parameter. Write the list as a manifest in a cache or bucket, then submit subtasks with small parameters such as `manifest_key="inputs/job-123.json"` and `shard=17`.
...
### Dependency groups and fan-out limits

When a task submits subtasks, Tilebox compacts subtasks with the same dependency shape into submission groups. A single task execution can submit up to 64 of these groups. This is often much higher than 64 subtasks: thousands of independent subtasks are one group, and thousands of subtasks that all depend on the same previous task are also one group.

Tilebox can handle large jobs, including jobs with millions of tasks. You do not need to reduce concurrency to scale. Instead, keep each task's submitted graph compact: use homogeneous stages, shared dependency shapes, stage-wrapper tasks, and small task inputs.

Stage wrappers can depend on each other. For example, a root task can submit a read-stage wrapper, a compute-stage wrapper that depends on the read stage, and an export-stage wrapper that depends on the compute stage. Each stage can submit its own homogeneous children, and the stage does not complete until its submitted descendants complete.

You run into the group limit when each subtask has a different dependency shape. Long chains and pairwise dependencies are common examples, because every task depends on a different predecessor.
...
For large fan-out, prefer stage-shaped graphs:

- submit many independent tasks together with `submit_subtasks` inside a stage-wrapper or shard task
- make the next stage depend on the previous stage wrapper when that expresses the stage barrier more cleanly than depending on every individual leaf task
- add a small barrier or reducer task between stages only when it makes the dependency shape simpler
- use recursive decomposition or shard tasks when you truly need many sequential or pairwise relationships, or when one task execution would submit too many children
